...
export default function AnalyticsPage() {
    ...
    const fetchAll = useCallback(async (days: number) => {
        ...
        try {
            const [s, p, h, ft] = await Promise.all([
                ...
                api.get(`/analytics/popular?limit=10&days=${days}`).then(r => r.data),
                api.get(`/analytics/hourly?days=${days}`).then(r => r.data),
                api.get(`/analytics/feedback-trend?days=${days}`).then(r => r.data),
            ]);
            setSummary(s);
    ...
    // Chờ cookieReady trước khi fetch — tránh gọi API trước khi cookie flush
    useEffect(() => {
        if (!cookieReady) return;
        fetchAll(selectedDays);
    }, [cookieReady, selectedDays]);
    ...
    const handleExport = async () => {
        setIsExporting(true);
        try {
            const res = await api.get(`/analytics/export?days=${selectedDays}`, {
                responseType: 'blob',
            });
